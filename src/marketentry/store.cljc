(ns marketentry.store
  "SSoT for the Benin (BEN) market-entry compliance actor, behind a
...
(defn demo-data
  ...
  `:mpme-status?` / `:base-price` / `:claimed-evaluated-price` are
  ground truth for the `mpme-preference-margin-mismatch` flagship check
  (Loi N°2020-26 Art. 77 al.3 mandatory bid-evaluation preference);
  `:requires-ifu?` / `:ifu-verified?` are ground truth for the
  conditional IFU check."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Zinsou Fabrication SARL" :portal "marches-publics.bj"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :mpme-status? true :base-price 1000000 :claimed-evaluated-price 950000.0
             :requires-ifu? true :ifu-verified? true
             :drafted? false :submitted? false
             :jurisdiction "BEN" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "marches-publics.bj"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :mpme-status? true :base-price 1000000 :claimed-evaluated-price 950000.0
             :requires-ifu? true :ifu-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Cotonou Systems SA" :portal "marches-publics.bj"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "BEN" :status :intake}
    "eng-4" {:id "eng-4" :operator "Porto-Novo Export Ltd" :portal "marches-publics.bj"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :mpme-status? false :base-price 1000000 :claimed-evaluated-price 950000.0
             :requires-ifu? true :ifu-verified? true
             :drafted? false :submitted? false
             :jurisdiction "BEN" :status :intake}
    "eng-5" {:id "eng-5" :operator "Parakou Logistics SARL" :portal "marches-publics.bj"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :mpme-status? true :base-price 1000000 :claimed-evaluated-price 950000.0
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               mpme-status? base-price claimed-evaluated-price
  ...
  (cond-> {:engagement/id id}
    ...
    (some? mpme-status?)                  (assoc :engagement/mpme-status? mpme-status?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/mpme-status? :engagement/base-price :engagement/claimed-evaluated-price
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :mpme-status? (boolean (:engagement/mpme-status? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-ben"})
