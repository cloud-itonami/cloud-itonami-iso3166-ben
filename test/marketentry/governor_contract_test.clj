  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Zinsou Fabrication SARL"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Zinsou Fabrication SARL" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest mpme-preference-margin-mismatch-is-held-and-unoverridable
  (testing "claimed evaluated price doesn't match the independently recomputed Art. 77 margin -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:mpme-preference-margin-mismatch} (-> (store/ledger db) last :basis)))
...
(deftest ifu-unverified-is-held-and-unoverridable
  (testing "unverified IFU (Identifiant Fiscal Unique) when required -> HARD hold"
    (let [[db actor] (fresh)
